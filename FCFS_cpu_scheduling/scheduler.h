...
public:
	...
	void Go() //This function initializes the scheduling process once all the threads are created.
	{
		...
		{ //cout<<"hey";
			++timer;
			cout<<"[GO]==============================================================================Timer value: "<<timer<<endl;
			for(unsigned int i = 0; i < num_cpu; ++i)
			{ //cout<<i<<endl;
				if (CPUs[i] != NULL)

				{ //cout<<"[GO]===== CPU not null"<<endl;
					...
					{ 
						cout << "[GO] Thread ID : " << CPUs[i]->tid << " Finishing Time : " << timer << " CPU No. : " << i << '\n';
						CPUs[i] = NULL;
					}
				}
			}	
		}
		cout<< "[GO]===End Timer:"<<timer<<endl;
		cout << "[GO]All the Threads have been executed !! .. Exiting Scheduler ..\n";
		system("pause");
	}
...
protected:
	...
	ThreadDescriptorBlock **CPUs; //Pointer array to let processors access the threads CPU[i]->tid;
